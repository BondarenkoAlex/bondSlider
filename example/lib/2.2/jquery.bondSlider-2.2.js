    ...
    $.fn.bondSlider=function(options){
        ...
        var initialization = (function(){
            ...
            function totalFramesAndWidthCont(){
                var frameW = 0;
				var frameH = 0;
                totalFrames = Math.ceil(frames$.length/o.sizeFrame);				
				for (var i=totalFrames-1, width = 0; i >= 0; i--) {
					if ( o.typeScroller == c.tpL.h ){
						var s = sizeFrame(frames$,i,o.sizeFrame);
						memory.widthCont = memory.widthCont + s;
						if ( memory.widthCont <= wrapFrames$.width() || i == totalFrames-1 ){
							memory.totalFrames = i+1;
						}
					}
                    else if ( o.typeScroller == c.tpL.v ){
                        frameH  =   frameH + sizeFrame(frames$,i,o.sizeFrame);
                        if ( frameH <= wrapFrames$.height() || i == totalFrames-1 ){
                            memory.totalFrames = i+1;
                        }
                    }						
				}
        ...
        var bindEvent = (function(){
            if (memory.totalFrames <= 1) return; /*нет кадров, листать нечего, выходим*/
            playAutoPlay();
            if ( nextBtn$ != null ){
